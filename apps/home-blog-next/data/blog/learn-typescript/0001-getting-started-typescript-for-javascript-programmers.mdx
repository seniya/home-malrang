---
title: 'Getting Started - TypeScript for JavaScript Programmers'
date: '2025-03-29 '
lastmod: '2025-03-29'
tags: ['typescript']
draft: false
summary: '이 문서는 TypeScript for JavaScript Programmers(공식)의 내용을 다시 정리한 문서입니다. 개인적인 학습 내용을 정리하기 위해 작성되었습니다.'
...
TypeScript는 몇 가지를 추가해 목록을 확장합니다. 예를 들어, `any` (무엇이든 허용합니다),
[unknown](https://www.typescriptlang.org/play/#example/unknown-and-never) (이 타입을 사용하는 사람이 타입이 무엇인지 선언했는가를 확인하십시오),
[never](https://www.typescriptlang.org/play/#example/unknown-and-never) (이 타입은 발생될 수 없습니다 `void` (`undefined`를 리턴하거나 리턴 값이 없는 함수).

타입을 구축하기 위한 두 가지 구문이 있다는 것을 꽤 빠르게 알 수 있을 것입니다.
: [Interfaces and Types](https://www.typescriptlang.org/play/?e=83#example/types-vs-interfaces) - interface를 우선적으로 사용하고!!
...
_참고_: `MyBool`위에 마우스를 올린다면, `boolean`으로 분류된 것을 볼 수 있습니다 - 구조적 타입 시스템의 프로퍼티며, 나중에 살펴보겠습니다.

유니언 타입이 가장 많이 사용된 사례 중 하나는 값이 다음과 같이 허용되는 `string` 또는 `number`의 [리터럴](https://www.typescriptlang.org/docs/handbook/literal-types.html)집합을 설명하는 것입니다:
...
- 전체 핸드북을 [처음부터 끝](https://www.typescriptlang.org/docs/handbook/intro.html)까지 읽으세요(30분)
- [Playground](https://www.typescriptlang.org/play#show-examples) 예시를 탐색하세요.
